...
fn draw_header(f: &mut Frame, app: &App, area: Rect) {
    ...
    let project_name = app.active_project.as_ref().map(|p| p.name.as_str()).unwrap_or("(none)");
...
fn draw_projects(f: &mut Frame, app: &App, area: Rect) {
    let header = Row::new(vec!["Name", "Path", "Created"])
        .style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD));

    let rows: Vec<Row> = app.projects.iter().enumerate().map(|(i, p)| {
    ...
    let table = Table::new(rows, [Constraint::Percentage(30), Constraint::Percentage(50), Constraint::Percentage(20)])
        .header(header)
        .block(Block::default().borders(Borders::ALL).title(" Projects [Enter=Select] "));

    f.render_widget(table, area);
}

fn draw_observations(f: &mut Frame, app: &App, area: Rect) {
    let header = Row::new(vec!["Type", "Scope", "Title", "State", "Updated", "Topic"])
        .style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD));

    let rows: Vec<Row> = app.observations.iter().enumerate().map(|(i, o)| {
        ...
        Row::new(vec![
            ...
            Cell::from(Span::styled(format!("{:?}", o.lifecycle), Style::default().fg(state_color))),
...
fn draw_sessions(f: &mut Frame, app: &App, area: Rect) {
    let header = Row::new(vec!["ID", "Started", "Ended", "Summary"])
        .style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD));
    ...
    let rows: Vec<Row> = app.sessions.iter().enumerate().map(|(i, s)| {
        ...
        let ended = s.ended_at.map(|t| t.format("%m-%d %H:%M").to_string()).unwrap_or_else(|| "active".to_string());
        let summary = s.summary.as_ref().map(|sum| truncate(&sum.goal, 50)).unwrap_or_default();
        Row::new(vec![
    ...
    let table = Table::new(rows, [Constraint::Length(14), Constraint::Length(14), Constraint::Length(14), Constraint::Min(20)])
        .header(header)
        .block(Block::default().borders(Borders::ALL).title(" Sessions [Enter=Detail] "));
...
fn draw_search(f: &mut Frame, app: &App, area: Rect) {
    ...
    let input = Paragraph::new(app.input_buffer.as_str())
        .style(input_style)
        .block(Block::default().borders(Borders::ALL).title(" Search [/=focus Enter=search Esc=unfocus] "));
    f.render_widget(input, chunks[0]);

    let header = Row::new(vec!["Score", "Type", "Title", "Content"])
        .style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD));

    let rows: Vec<Row> = app.search_results.iter().enumerate().map(|(i, r)| {
    ...
    let table = Table::new(rows, [Constraint::Length(8), Constraint::Length(12), Constraint::Min(20), Constraint::Min(30)])
        .header(header)
...
fn draw_observation_detail(f: &mut Frame, app: &App, area: Rect) {
    ...
    let content = Paragraph::new(obs.content.as_str())
        ...
        .block(Block::default().borders(Borders::ALL).title(format!(" {} ", obs.title)));
    f.render_widget(content, chunks[0]);

    let meta_lines = vec![
        Line::from(vec![Span::styled("ID: ", Style::default().fg(Color::Yellow)), Span::raw(&obs.id)]),
        Line::from(vec![Span::styled("Type: ", Style::default().fg(Color::Yellow)), Span::raw(format!("{:?}", obs.memory_type))]),
        Line::from(vec![Span::styled("Scope: ", Style::default().fg(Color::Yellow)), Span::raw(format!("{:?}", obs.scope))]),
        Line::from(vec![
            Span::styled("State: ", Style::default().fg(Color::Yellow)),
            Span::styled(format!("{:?}", obs.lifecycle), Style::default().fg(lifecycle_color(obs.lifecycle))),
        ]),
        Line::from(vec![Span::styled("Hash: ", Style::default().fg(Color::Yellow)), Span::raw(truncate(&obs.hash, 16))]),
        Line::from(vec![Span::styled("Revisions: ", Style::default().fg(Color::Yellow)), Span::raw(obs.revision_count.to_string())]),
        Line::from(vec![Span::styled("Duplicates: ", Style::default().fg(Color::Yellow)), Span::raw(obs.duplicate_count.to_string())]),
        Line::from(vec![Span::styled("Created: ", Style::default().fg(Color::Yellow)), Span::raw(obs.created_at.format("%Y-%m-%d %H:%M").to_string())]),
        Line::from(vec![Span::styled("Updated: ", Style::default().fg(Color::Yellow)), Span::raw(obs.updated_at.format("%Y-%m-%d %H:%M").to_string())]),
        Line::from(vec![Span::styled("Last Seen: ", Style::default().fg(Color::Yellow)), Span::raw(obs.last_seen_at.format("%Y-%m-%d %H:%M").to_string())]),
        Line::from(vec![
            Span::styled("Review After: ", Style::default().fg(Color::Yellow)),
            Span::raw(obs.review_after.map(|d| d.format("%Y-%m-%d").to_string()).unwrap_or_else(|| "—".to_string())),
        ]),
        Line::from(vec![
        ...
        Line::from(vec![
            ...
            Span::raw(if obs.tags.is_empty() { "—".to_string() } else { obs.tags.join(", ") }),
        ]),
        Line::from(""),
...
fn draw_session_detail(f: &mut Frame, app: &App, area: Rect) {
    ...
    let ended = sess.ended_at.map(|t| t.format("%Y-%m-%d %H:%M").to_string()).unwrap_or_else(|| "active".to_string());
    let summary_text = sess.summary.as_ref().map(|s| {
        format!(
            "Goal: {}\nDiscoveries: {}\nAccomplished: {}\nNext: {}",
            s.goal,
            s.discoveries.join(", "),
    ...
    let header = Row::new(vec!["Type", "Title", "Created"])
        .style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD));

    let rows: Vec<Row> = app.detail_session_observations.iter().map(|o| {
        Row::new(vec![
            Cell::from(format!("{:?}", o.memory_type)),
    ...
    let table = Table::new(rows, [Constraint::Length(12), Constraint::Min(30), Constraint::Length(14)])
        .header(header)
        .block(Block::default().borders(Borders::ALL).title(format!(
...
fn draw_confirm_dialog(f: &mut Frame, app: &App, area: Rect) {
    let msg = match &app.confirm_action {
        Some(ConfirmAction::HardDelete(id)) => format!("Hard-delete observation {}?\n\nThis cannot be undone.\n\n[y] Yes  [n/Esc] No", truncate(id, 20)),
        None => return,
    };
...
fn draw_status_bar(f: &mut Frame, app: &App, area: Rect) {
    let keybinds = match app.detail {
        DetailView::None => "q:Quit  Tab:Switch  ↑↓:Nav  Enter:Select  /:Search  d:Del  D:HardDel  r:Review  n/p:Page",
        _ => "Esc:Back  q:Quit",
    };
