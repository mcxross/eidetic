...
impl MemSave {
    ...
    pub async fn mem_save(
        ...
        let content_hash = Observation::compute_hash(&project_id, &scope, &params.memory_type, &params.title, &params.content);
        ...
        if let Some(existing) = all_obs.iter_mut().find(|o| 
            ...
            self.store.storage().update_observation(existing).await.map_err(|e| McpError::internal_error(e.to_string(), None))?;
        ...
        if let Some(topic) = &params.topic_key {
            if let Some(existing) = all_obs.iter_mut().find(|o| o.scope == scope && o.topic_key.as_ref() == Some(topic)) {
        ...
        let review_after = if let Some(ra) = params.review_after {
            chrono::DateTime::parse_from_rfc3339(&ra).ok().map(|dt| dt.with_timezone(&chrono::Utc))
        } else {
            None
        };
        ...
        obs.metadata = params.metadata.unwrap_or_default().as_object().map_or(HashMap::new(), |m| {
        ...
        if obs.capture_prompt {
            ...
            if let Ok(prompts) = self.store.storage().get_prompts(&project_id, active_session.as_ref()).await {
        ...
        if let Some(sid) = &obs.session_id {
            if let Some(mut session) = self.store.storage().get_session(sid).await.map_err(|e| McpError::internal_error(e.to_string(), None))? {
                session.observation_ids.push(obs.id.clone());
                self.store.storage().update_session(&session).await.map_err(|e| McpError::internal_error(e.to_string(), None))?;
            }
        }
