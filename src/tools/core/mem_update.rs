...
impl MemUpdate {
    ...
    pub async fn mem_update(
        ...
        let mut obs = self.store.storage().get_observation(&params.id).await.map_err(|e| McpError::internal_error(e.to_string(), None))?
        ...
        if let Some(review_after) = params.review_after {
            obs.review_after = chrono::DateTime::parse_from_rfc3339(&review_after).ok().map(|dt| dt.with_timezone(&chrono::Utc));
        }
