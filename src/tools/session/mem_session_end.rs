    ...
    pub async fn mem_session_end(
        ...
        let session_id = if let Some(sid) = params.session_id {
            ...
            self.store.get_current_session().await
                .ok_or_else(|| McpError::invalid_params("No active session found. Provide session_id or start a session first.".to_string(), None))?
        };

        let mut session = self.store.storage().get_session(&session_id).await.map_err(|e| McpError::internal_error(e.to_string(), None))?
            .ok_or_else(|| McpError::invalid_params(format!("Session not found: {}", session_id), None))?;
