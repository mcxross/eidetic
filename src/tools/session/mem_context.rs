...
impl MemContext {
    ...
    pub async fn mem_context(
        ...
        let output = format!(
            ...
            context.recent_sessions.iter().map(|s| format!(
                "  - {} ({}) {}",
                s.id,
            ...
            context.recent_observations.iter().map(|o| format!(
                "  - [{:?}] {} - {}",
                o.memory_type, o.title, o.content.chars().take(80).collect::<String>()
            )).collect::<Vec<_>>().join("\n"),
            context.active_topics.len(),
            if context.active_topics.is_empty() { "none".to_string() } else { context.active_topics.join(", ") }
        );
